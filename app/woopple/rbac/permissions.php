<?php

return [
    'oAccessAdminPanel' => [
        'type' => 2,
        'description' => 'Доступ к панели администрирования'
    ],
    'oAccessUserManagement' => [
        'type' => 2,
        'description' => 'Доступ к просмотру раздела пользователей в панели администратора'
    ],
    'oCreateUser' => [
        'type' => 2,
        'description' => 'Доступ к созданию пользователей'
    ],
    'oModifyUser' => [
        'type' => 2,
        'description' => 'Доступ к обновлению данных пользователей'
    ],
    'oBlockUser' => [
        'type' => 2,
        'description' => 'Доступ к блокировке пользователей'
    ],
    'oUnblockUser' => [
        'type' => 2,
        'description' => 'Доступ к разблокировке пользователей'
    ],
    'oAccessSecurityControl' => [
        'type' => 2,
        'description' => 'Доступ в раздел управления безопасностью'
    ],
    'oAccessRestorePasswordControl' => [
        'type' => 2,
        'description' => 'Доступ в раздел просмотра заявок на сброс пароля'
    ],
    'oRestorePassword' => [
        'type' => 2,
        'description' => 'Доступ к сбросу пароля'
    ],
    'oAccessBlacklistControl' => [
        'type' => 2,
        'description' => 'Доступ к разделу блокировки пользователей'
    ],
    'oAccessDepartmentControl' => [
        'type' => 2,
        'description' => 'Доступ к управлению отделами'
    ],
    'oCreateDepartment' => [
        'type' => 2,
        'description' => 'Создание нового отдела'
    ],
    'oDeleteDepartment' => [
        'type' => 2,
        'description' => 'Удаление существующего отдела'
    ],
    'oModifyDepartment' => [
        'type' => 2,
        'description' => 'Изменение существующего отдела'
    ],
    'oAccessHumanResourceSection' => [
        'type' => 2,
        'description' => 'Доступ к разделу HR'
    ],
    'oAccessHREmployersSection' => [
        'type' => 2,
        'description' => 'Доступ к разделу управления сотрудниками компании'
    ],
    'oAccessHRBeginnersSection' =>  [
        'type' => 2,
        'description' => 'Доступ к разделу управления новичками компании'
    ],
    'oFillProfile' => [
        'type' => 2,
        'description' => 'Заполнение профиля новичка'
    ],
    'oViewPersonalSection' => [
        'type' => 2,
        'description' => 'Доступ к разделу управления персоналом'
    ],
    'oViewStructureSection' => [
        'type' => 2,
        'description' => 'Доступ к разделу орг. структуры'
    ],
    'oViewDepartmentList' => [
        'type' => 2,
        'description' => 'Доступ к просмотру существующих отделов'
    ],
    'oViewTeamList' => [
        'type' => 2,
        'description' => 'Доступ к просмотру существующих команд'
    ],
    'oAccessLeadSection' => [
        'type' => 2,
        'description' => 'Доступ к разделу руководителя'
    ],
    'oTestsControl' => [
        'type' => 2,
        'description' => 'Доступ к управлению тестированиями'
    ],
];